...
export default function BasicDemo() {
    ...
    useEffect(() => {
        if(data && data.getAlojamientos){
            setProducts(data.getAlojamientos)
        }
      ...
      const ciudadVillavicencio = products.filter((numero) => {
        return numero.ciudad === "villavicencio" ;
      });
    ...
    const getSeverity = (ciudadVillavicencio) => {
        switch (ciudadVillavicencio.inventoryStatus) {
            case 'INSTOCK':
                return 'success';
